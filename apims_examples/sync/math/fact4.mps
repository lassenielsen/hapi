(nu prod : 1=>2<Int>;                    // Send argument: x1
           1=>2<Int>;                    // Send argument: x2
           2=>1<Int>;Gend)               // Send result: x1*...*x2
( def Prod() =                           // Define SubFact Service {{{
    ...
    | s[1] >> x1;                        // Receive x1
      ...
      then link(2,prod,s1,1);            // Connect to (another) SubFact Server
           s1[2] << x1;                  // Send x1
           s1[2] << (x1+x2)/2;           // Send average of x1 and x2 (x3)
           link(2,prod,s2,1);            // Connect to (a third) SubFact Server
           s2[2] << (x1+x2)/2 +1;        // Send x3+1
           s2[2] << x2;                  // Send x2
           s1[2] >> fact1;               // Receive x1*...*x3
           s2[2] >> fact2;               // Receive x3+1*...*x2
           s[1] << fact1*fact2;          // Send x1*...*x3*(x3+1)*...*x2
           end
      else if x1 = x2                    // If trivial (x1=x2)
      then s[1] << x1;                   // Send x1 (=x2)
           end
      else s[1] << 1;                    // Send 1 (if x1>x2)
           end
    )
