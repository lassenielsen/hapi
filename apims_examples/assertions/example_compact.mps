define $seq bit1 bit2 bit3 bit4 =
  rec $state<x1:Bool=bit1,x2:Bool=bit2,x3:Bool=bit3,x4:Bool=bit4>.
         {^Continue[[not x1 or not x2 or not x3 or not x4]]:
           1=>2:1<Int>;
           $state<not x1,
          ...
          ^End[[x1 and x2 and x3 and x4]]:
           Gend
         }
in
(nu a: $seq<false,false,false,false>)
(nu xor: 1=>2:2<Bool> as x; 1=>2:2<Bool> as y; 2=>1:1<Bool> as z[[(z or (x and y) or (not x and not y)) and (not z or (x and not y) or (not x and y))]]; Gend)
( def XOR() =
    link(2,xor,s,2);
    ( XOR()
    | s[2]>>x1;
...
| link(2,a,s,1);
  ...
  in
    SEQ1<false,false,false,false>(s,0)
)
