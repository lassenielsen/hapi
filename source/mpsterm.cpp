...
{ _compile_id=1;
  ...
  result
    ...
    << "            (*task)->Mark(myGCMarks);\n"
    << "          ourGCReady->insert(this); // GC results are ready\n"
    << "          myGCNewValues.clear();\n"
    << "        }\n"
    ...
    << "{ \n"
    << "  std::atomic<std::set<libpi::task::Worker*> > gcready;\n"
    << "  libpi::task::Worker::SetGCReady(&gcready);\n"
    << "\n"
    ...
    << "      }\n"
    << "      usleep(100000);\n"
    << "    } while (gcready.size()<libpi::task::Worker::Workers); // Continue until all marks are in\n"
    << "    for (vector<libpi::task::Worker*>::iterator w=workers.begin(); w!=workers.end(); ++w)\n"
    << "      (*w)->GCWait();\n"
    ...
    << "    }\n"
    << "    set<libpi::Value*> done=values;\n"
    << "    done.erase(marks.begin(),marks.end());\n"
    << "    for (set<libpi::Value*>::iterator val=done.begin(); val!=done.end(); ++val)\n"
    << "      delete *val;\n"
    << "    values.erase(done.begin(),done.end());\n"
    << "  }\n"
