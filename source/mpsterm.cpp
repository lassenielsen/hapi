...
{ _compile_id=1;
  ...
  for (set<string>::const_iterator id=ids.begin(); id!=ids.end(); ++id)
    result << "      if (var_" << ToC_Name(*id) << "!=NULL) var_" << ToC_Name(*id) << "->Mark(marks);" << endl;
  result
  << "    }" << endl
  << "};" << endl;
  for (MpsFunctionEnv::const_iterator def=defs.begin(); def!=defs.end(); ++def)
  ...
  result
    ...
    << "  }\n"
    << "  // Perform GC until program is complete\n"
    ...
    << "    else if (string(argv[i])==\"-pi_workers\" && i+1<argc)\n"
    << "      libpi::task::Worker::Workers=atoi(argv[++i]);\n"
    << "    else if (string(argv[i])==\"-pi_gc_interval\" && i+1<argc)\n"
    << "      _gc_interval=atoi(argv[++i]);\n"
    << "    else\n"
