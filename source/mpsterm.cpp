...
{ _compile_id=1;
  ...
  result
    ...
    << "  return NULL;\n"
    << "} // }}}\n"
    << "void *_clean_garbage(void *arg) // {{{\n"
    << "{ libpi::Value *obj=NULL;\n"
    << "  libpi::Value::GetGarbage(&obj);\n"
    << "  while (obj)\n"
    << "  { delete obj;\n"
    << "    libpi::Value::GetGarbage(&obj);\n"
    << "  }\n"
    ...
    << "{ pthread_t x[libpi::task::Worker::TargetTasks+1];\n"
    << "  pthread_attr_t y;\n"
    << "  pthread_attr_init(&y);\n"
    ...
    << "  }\n"
    << "  pthread_create(&(x[libpi::task::Worker::TargetTasks]),&y,_clean_garbage,(void*)NULL);\n"
    << "  for (size_t wc=0; wc<libpi::task::Worker::TargetTasks; ++wc)\n"
    << "    pthread_join(x[wc],NULL);\n"
    << "  libpi::Value::Erase(NULL);\n"
    << "    pthread_join(x[libpi::task::Worker::TargetTasks],NULL);\n"
    << "} // }}}\n"
