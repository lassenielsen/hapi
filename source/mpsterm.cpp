...
{ _compile_id=1;
  ...
  string result = (string)
       ...
       + "int *__system_aprocs=(int*)mmap(NULL, sizeof(int), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0); // Actual number of active processes\n"
       + "inline void IncAprocs() // {{{\n"
       + "{ pthread_mutex_lock(__system_aprocs_mutex);\n"
       + "  ++(*__system_aprocs);\n"
       + "  pthread_mutex_unlock(__system_aprocs_mutex);\n"
       + "} // }}}\n"
