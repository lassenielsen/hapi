...
string MpsLink::ToC(const string &taskType) const // {{{
{
  ...
  if (myPid==1) // Orchestrate linking
  {
    ...
    for (size_t i=0; i<myMaxpid-1; ++i)
      ...
      << "        if (!((libpi::task::Link*)_this->var_" << ToC_Name(myChannel) << ")->GetChannels()[" << i << "]->Receive(_task,_task->tmp))" << endl
      << "          return false;" << endl
      << "        " << rcvLabels[i] << ":" << endl
  ...
  else // Send Channel and receive session
  {
    ...
    result
    << "    { _task->tmp=new libpi::task::Channel(NULL); //Channel will be in session, and deleted when session is deleted" << endl
